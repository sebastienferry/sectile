...
- **Prometheus metrics.** The server exposes `/metrics` on its own port (`:8093`, set with `SECTILE_METRICS_ADDR`, `off` to disable): requests, latency and errors per controller, active users, active runs by status, and the build version.
...
- **The status bar no longer shows the MCP clients.** The connected-clients indicator and its panel are gone from the status bar; who is using the board is now on the Administration page. `GET /api/mcp/sessions` still lists the live sessions.
