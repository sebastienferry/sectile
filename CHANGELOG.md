...
- **Open a saved view from the desktop app.** *Tasks list* offers your saved views next to your projects. A view's open tasks show in the tickets pane with their project, and each one launches with its own project's skills and settings. A view can name the Git repository its work lives in, from its form on the web, and a local folder on each workstation, from its tickets pane. Launches from the view run in that folder, or in your checkout of the view's repository, and so do their relaunches and next steps; a folder that is a checkout of another repository is kept, with a warning. A pull request in the view's repository then counts for the tickets launched from it, mono-repo projects included, and the synchronisation finds it by branch when the ticket has none. With an older agent or server, or without saved views, the desktop shows projects only. (#429)
