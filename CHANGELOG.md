...
- **A locked personal GitHub token stops the call instead of borrowing the
  ...
  account they did not choose. Such a call now fails and says the credential is
  locked, as Jira already did. Somebody who stored no GitHub token at all still
  uses the project or server token.
