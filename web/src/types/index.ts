...
export interface SkillEditorEntry {
  ...
  repoContent?: string
  repoPath?: string
  /** D'où vient la ligne de base, sous l'édition éventuelle du projet. */
  origin?: 'builtin' | 'marketplace' | string
  /** Coordonnées du pack, « marketplace/plugin@version+sha », quand il y en a un. */
  packOrigin?: string
}

/** Une source enregistrée de corps de skills : un dépôt git, ou un répertoire. */
export interface SkillMarketplace {
  name: string
  kind: 'github' | 'git' | 'path' | string
  locator: string
  owner?: string
  description?: string
  lastCommit?: string
  ...
  createdAt?: string
  /** Projets qui épinglent un plugin de cette marketplace. */
  pinnedBy?: string[]
}
...
export interface MarketplacePlugin {
  ...
  description?: string
  version?: string
  /** Répertoires de skills du workflow que ce plugin fournit. */
  skills: string[]
  /** Répertoires que Sectile n'installe pas : signalés, jamais supprimés en silence. */
  ignored?: string[]
  /** Pourquoi ce plugin ne fournit rien d'utilisable. */
  error?: string
}
...
export interface SkillPack {
  ...
  version?: string
  commit?: string
  /** Corps du SKILL.md, par nom de répertoire de skill. */
...
export interface SkillPackPreviewEntry {
  skillId: string
  dirName: string
  name: string
  current: string
...
export interface SkillPackPreview {
  pack: SkillPack
  entries: SkillPackPreviewEntry[]
  /** Skills du workflow que le pack ne fournit pas : elles gardent le modèle intégré. */
  missing?: string[]
}

export interface SkillPackPin {
  projectId: string
  marketplace: string
  plugin: string
  version?: string
  ...
  appliedAt?: string
  skills: string[]
  /** La marketplace n'est plus enregistrée : les corps appliqués restent, les coordonnées ne résolvent plus. */
  orphaned?: boolean
}
