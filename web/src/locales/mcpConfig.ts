export const mcpConfigText = {
  "fr": {
    ...
    "transport": "Transport",
    "target": "Connexion",
    "proxyUrl": "URL du proxy local",
    "local": "Le moteur utilise le proxy HTTP local sans clé client. Activez ce mode dans Sectile Desktop et recopiez son URL (le port peut varier). L’agent local doit rester actif ; les processus de ce poste peuvent accéder à MCP avec son identité.",
    "stdioLocal": "Le moteur lance sectile-agent et échange via stdin/stdout. La passerelle transmet les appels au proxy local. Utilisez le chemin du binaire installé ; l’agent local doit rester actif.",
    "remote": "HTTP distant (défaut)",
    "proxy": "HTTP local · proxy",
    "choose": "Configuration à fusionner dans",
  ...
  "en": {
    ...
    "local": "The engine uses the local HTTP proxy without a client key. Enable this mode in Sectile Desktop and copy its URL (the port may vary). The local agent must stay running; processes on this workstation can access MCP using its identity.",
    "stdioLocal": "The engine starts sectile-agent and exchanges messages over stdin/stdout. The bridge forwards calls to the local proxy. Use the installed binary path; the local agent must stay running.",
    "remote": "Remote HTTP (default)",
    "proxy": "Local HTTP · proxy",
    "choose": "Configuration to merge into",
