...
test('command presets include default, Claude, AGY and Codex', () => {
  ...
  assert.match(agy.autonomous, /\| jq --unbuffered -Rr/)
  ...
  assert.match(claude.autonomous, /\| jq --unbuffered -Rr/)

  const codex = presets.find(p => p.label === 'Codex')
  assert.ok(codex)
  assert.match(codex.cmd, /codex --model \{model\}/)
  assert.match(codex.autonomous, /codex exec --json --model \{model\}/)
  assert.match(codex.autonomous, /\| jq --unbuffered -Rr/)
})
