// Isolated browser regression: real TaskCard, its children and styles; useApp is mocked in every component.
// The card is condensed through its `compact` prop, as BoardView passes it in the condensed board mode.
...
await page.getByRole('button',{name:'Actions',exact:true}).click();await page.getByRole('button',{name:'Avancer une étape',exact:true}).click();await page.getByRole('button',{name:'Actions',exact:true}).click();assert(await page.getByRole('button',{name:'Chaîne complète'}).isDisabled());await page.keyboard.press('Escape');await page.waitForTimeout(600);
await page.getByRole('button',{name:'Actions',exact:true}).click();await page.getByRole('button',{name:'Chaîne complète'}).click();assert.deepEqual(await page.evaluate(()=>calls.filter(c=>c[0]==='advance').map(c=>c.slice(0,3))),[['advance','fixture',false],['advance','fixture',true]]);
