...
window.allTasks=[task('new-low','new','low'),task('clarified','clarified','urgent'),task('specified','specified','medium'),task('new-high','new','high')];
window.ctx={tasks:allTasks,currentProject:{id:'p'},activities:[],activeTasks:new Set(),boardGrouping:'workflow',hideDone:false,isPinned:()=>false,t:translations.fr,
...
try {
  ...
  console.log('PASS: Backlog eligibility, visible group/sort order, failure retention, accepted clearing, pending guard, hidden tasks, cross-project selection')
} finally {
  await browser?.close()
  await server.close()
}
