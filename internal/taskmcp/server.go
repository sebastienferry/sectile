// Package taskmcp exposes TaskFlow's existing workflow services as typed MCP tools.
package taskmcp
...
func NewServer(database *db.DB) *mcp.Server {
	s := mcp.NewServer(&mcp.Implementation{Name: "taskflow", Version: "1.0.0"}, nil)
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_get_task", Description: "Read task details, workflow labels, branch metadata and live comments."},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_transition_stage", Description: "Record a verified standalone workflow stage, optionally attach the task pull request or merge request URL using prUrl, and queue tracker synchronization. Managed runs must use their result contract.", InputSchema: map[string]any{
	...
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_add_comment", Description: "Post a comment to the task's discussion on its tracker or local board."},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_list_tasks", Description: "List board tasks, optionally filtered by project, status and sprint."},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_get_project_context", Description: "Read project description, repository identity, execution settings, specification framework and skill instructions. Supply projectId or taskKey; read repository AGENTS.md locally for additional conventions."},
	...
	mcp.AddTool(s, &mcp.Tool{Name: "taskflow_finish_run", Description: "Finish the specified remote execution with completed, failed or canceled status. Call when the entire invoked skill ends, including when stopping for user input. Does not transition the task."},
