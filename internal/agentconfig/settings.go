...
func WriteSettings(settings Overrides) error {
	...
	for _, key := range []string{"disconnectedProjects", "projects", "worktrees", "parallelism", "commands", "aiProvider", "aiCommandTemplate", "aiModel", "aiSkillModels", "terminal", "skills"} {
		delete(fields, key)
	}
	for _, key := range []string{"projects", "worktrees", "parallelism", "commands"} {
