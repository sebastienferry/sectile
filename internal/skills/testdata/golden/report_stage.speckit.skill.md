...
3. In a managed run, record nothing per step and call no transition or comment tool: only the owner of the invocation writes the result through the supplied contract, once, when the whole invocation ends.
