...
// credentialHandler is a handler with no identity provider configured, so
// webSessionUser answers the implicit user and the routes act on one identity
// throughout, which is what these tests are about.
func credentialHandler(t *testing.T) *Handler {
	t.Helper()
...
func TestPersonalCredentialRoutesNeverReturnAToken(t *testing.T) {
	h := credentialHandler(t)

	store := func(body string) *httptest.ResponseRecorder {
		w := httptest.NewRecorder()
		h.HandleUserTrackerCredentials(w, httptest.NewRequest(http.MethodPut, "/api/me/tracker-credentials", strings.NewReader(body)))
		return w
	}
	...
	h.HandleUserTrackerCredentials(lock, httptest.NewRequest(http.MethodPost, "/api/me/tracker-credentials/lock", strings.NewReader(`{"tracker":"jira"}`)))
	if lock.Code != http.StatusOK || !strings.Contains(lock.Body.String(), `"unlocked":false`) {
		t.Fatalf("lock: %d %s", lock.Code, lock.Body)
	}

	wrong := httptest.NewRecorder()
	h.HandleUserTrackerCredentials(wrong, httptest.NewRequest(http.MethodPost, "/api/me/tracker-credentials/unlock", strings.NewReader(`{"tracker":"jira","passphrase":"nope"}`)))
	if wrong.Code != http.StatusForbidden {
		t.Fatalf("a wrong passphrase must be refused: %d %s", wrong.Code, wrong.Body)
	}

	right := httptest.NewRecorder()
	h.HandleUserTrackerCredentials(right, httptest.NewRequest(http.MethodPost, "/api/me/tracker-credentials/unlock", strings.NewReader(`{"tracker":"jira","passphrase":"open sesame"}`)))
	...
	h.HandleUserTrackerCredentials(gone, httptest.NewRequest(http.MethodDelete, "/api/me/tracker-credentials?tracker=jira", nil))
