...
func validateSkillResult(result *skillResult, skillID, repoPath string, prLookup func(string, string) string) error {
	s := struct{ ID string }{models.NormalizeSkillID(skillID)}
	switch s.ID {
	case "clarify", "specify", "implement", "adjust", "create_pr", "pickup", "handoff":
	default:
		return fmt.Errorf("unknown workflow skill %s", skillID)
	}
