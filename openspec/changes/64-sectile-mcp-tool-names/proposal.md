...
## Why
The product is named Sectile, but MCP discovery and managed client registrations still expose TaskFlow. GitHub #64 requests a consistent Sectile service identity and generic tool names. The accepted clarification explicitly removes legacy tool support on both agent and server.

## What Changes
- Identify HTTP and stdio MCP servers and managed registrations as `sectile`.
- Remove `taskflow_` from all eight tool names; reject old calls without aliases or fallback dispatch.
...
## Decision Source
`docs/clarifications/64.md` and the accepted clarification on https://github.com/sebastienferry/taskflow/issues/64. No open product questions remain. The MCP naming decision supersedes the compatibility approach of change #47 only for MCP tool names and managed service registration.
