...
Expand recognized tokens in a single pass over the original template. Preserve shell argument data in unquoted, single-quoted and double-quoted arguments, including embedded and repeated tokens. Double-quoted arguments temporarily close their quotes around a single-quoted value, also avoiding interactive shell history expansion of exclamation marks. Do not recursively process substituted values. Keep unknown tokens literal and the existing required prompt token. This is argument interpolation in trusted shell templates, not a general shell parser or support for placeholders as executable shell syntax.
