...
## What Changes
- Persist the board card display mode (`condensed` vs `expanded`) in browser `localStorage` under `sectile_board_display_mode` (with fallback to `taskacao_board_display_mode`).
