...
- [ ] 4.4 Remove the `osascript` alert from both scripts: the hook is a reporter, and the banner is the desktop's (D7).
- [ ] 4.5 Report the run state to `<loopback>/control/runs/{id}/waiting` when the session carries a run, with a bounded timeout.
- [ ] 4.6 When the session carries no run, read `~/.taskflow/agent-connection.json` for the loopback URL and credential, and post a session-level report naming the session by its `cwd` basename. A workstation with no such file is a silent no-op (D9).
- [ ] 4.7 Shell-level tests: both scripts exit 0 and print nothing with an unparseable payload, with no connection file, and against a dead loopback; the session name is the `cwd` basename and falls back when absent.
...
- [ ] 6.1 Add one module naming each run state with its glyph and its colour, as the single definition both the web badge and the desktop notification read (D8).
- [ ] 6.2 Have `web/src/components/RemoteRunBadge.tsx` take its glyph and colour from that module instead of its local `PRESENTATION` map.
- [ ] 6.3 Provide each state's glyph in a form the desktop can put on a notification, generated from the same definition rather than maintained beside it.
- [ ] 6.4 Tests: every state the indicator can return has a definition, and the badge and the notification resolve the same glyph for the same state.
...
- [ ] 8.1 Add `waitingSince` to the desktop run payload, set and cleared by the loopback waiting handler, so `/desktop/runs` carries it.
- [ ] 8.2 Hold session-level reports from sessions Sectile did not launch in a bounded list on the daemon, drained by the desktop on its existing poll (D9).
- [ ] 8.3 Raise the notification from the desktop through Electron's notification API, on the transition rather than the state: not-waiting to waiting, and running to a terminal status.
- [ ] 8.4 Put the shared glyph for the state on the notification, and name the session by its task or its directory.
- [ ] 8.5 Check notification availability once and degrade to the list alone when it is denied, without prompting on every event.
- [ ] 8.6 Desktop tests: a transition notifies, a repeated poll of the same state does not, a terminal status notifies once, and a denied permission raises nothing and throws nothing.

## 9. Documentation
- [ ] 9.1 Update the `docs/` pages for the revised notification path.
- [x] 9.2 Add an ADR for the waiting-state transport and the decision to model it as a timestamp rather than a status.
- [ ] 9.3 Extend that ADR with the revision: why the banner moved from `osascript` in the hook to the desktop application, and the shared icon vocabulary.
- [x] 9.4 Add an entry to `.agents/MEMORY.md` recording that Sectile now writes `~/.claude/settings.json`.

## 10. Validation
- [ ] 10.1 `make test` (Go suite, web tests, `tsc --noEmit`, `oxlint`) and the desktop test suite.
- [ ] 10.2 Manual check: a permission prompt raises a notification attributed to Sectile carrying the waiting glyph, the end of a turn raises the other, and the run shows as waiting then back to running in the UI.
- [ ] 10.3 Manual check that a hook failure, a stopped local agent and a closed desktop all leave the session running normally.
