...
Both supported MCP transports SHALL identify their server as `sectile` and expose exactly these nine tools: `get_task`, `transition_stage`, `add_comment`, `list_tasks`, `get_project_context`, `list_projects`, `start_run`, `finish_run`, and `create_task`. First-party bridge and desktop MCP client identities SHALL use Sectile names.
...
- **THEN** the server name is `sectile` and the catalog contains exactly the nine canonical names
...
- **GIVEN** a stdio relay whose server advertises a catalog that is not exactly the nine canonical names
