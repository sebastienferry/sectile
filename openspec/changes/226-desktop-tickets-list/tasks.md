...
- [x] 4.8 Hold the open menu's closer on the view so a rebuild or a disconnection detaches its document listener, and carry composed instructions and their execution mode across a rebuild.
- [x] 4.7 Close the pane from `select()` and from `agentUnavailable()`, refuse to open it with no agent connected, run `renderTicketRows()` on the deferred render path, restore focus to the opener when it is still reachable, keep the unconfigured notice on an empty list, clear the status line on a new search, and move focus to the row menu when the focused **Run** is disabled.
