...
- Add a "Générer les tickets Sectile" action button in `RoadmapView.tsx` to convert proposed macro breakdown items directly into Sectile task cards linked via `parentKey`.

## Capabilities

### New Capabilities
- `interactive-macro-refiner`: Interactive LLM workflow allowing users to clarify vague macro framing text through live TTY dialogue and automatically break down refined macros into concrete Sectile tickets.
