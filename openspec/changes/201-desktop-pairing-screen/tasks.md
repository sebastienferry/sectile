...
- [x] 1.1 Read `desktop/src/main.js` and confirm the connect form carries a `Pairing code` field and
      the API key behind an `Advanced` disclosure, with no required `Server token`.
- [x] 1.2 Read `desktop/electron/pairing.cjs` and confirm `resolveConnectCredential` gives the code
      priority and raises a message naming both ways in when neither is filled.
- [x] 1.3 Read `desktop/electron/main.cjs` and confirm the URL is validated before the credential is
      resolved, and that `storeKey` persists with and without a keyring.
- [x] 1.4 Run `cd desktop && node --test tests/*.test.cjs` and `npm run test:ui`; note that the UI
      tests need `npx vite build` first, since they load `dist/index.html`.

## 2. Close any gap the verification exposes
- [x] 2.1 For each scenario of `specs/desktop-pairing/spec.md` with no covering test, add one.
- [x] 2.2 Fix any behaviour that diverges from the specification, in the smallest possible change.
      No behaviour diverged. The credential persistence was extracted from `main.cjs` into
      `electron/credential-store.cjs`, unchanged, so the two keyring paths can be tested at all.
