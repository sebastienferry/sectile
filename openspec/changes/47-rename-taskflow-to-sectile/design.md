...
The product clarification for ticket #47 authorizes renaming TaskFlow to **Sectile** (slug: `sectile`) across public branding, build/distribution targets, package metadata, and documentation while retaining backwards compatibility with existing environments and configurations.
...
| Instructions & templates | Update default product prose in `internal/db/projectcontext.go`, `internal/db/skilltemplates.go`, `internal/runner/runner.go`, and generated workflow skill templates. Delimiters (`taskflow:project-context`) remain unchanged. |
...
| --- | --- |
| Database path resolution | Retain existing database search order: explicit `DB_PATH` > working-directory `./tasks.db` > user-config `$APP_DIR/taskflow/tasks.db` > user-config `$APP_DIR/taskacao/tasks.db` > fallback default `$APP_DIR/taskflow/tasks.db`. Retain SQLite WAL/SHM file structure and schema. No database file relocation or schema migration. |
| Environment files | Retain existing precedence: process environment > `.env` > `.env.local` > `$APP_DIR/taskflow/.env`. No breaking discovery path changes. |
| Environment variables | Retain existing `TASKFLOW_*` and `TASKACAO_*` environment variable support and precedence. Child processes and custom terminal commands continue receiving `TASKFLOW_*` variables. |
| Project files | Retain `.taskflow/` configuration directory, config JSON schema, `.tasks/` worktree paths, and `taskflow:project-context` delimiters in instruction files. |
| Browser state | Retain `taskflow_*` localStorage keys and existing `taskacao_*` read fallbacks. Do not introduce broken or fragmented keys. |
| Health and machine protocols | Retain health check service identifier (`taskflow-api`), API route paths, machine-facing terminal markers (`__TASKFLOW_*`), and task result contracts. Update only human-readable presentation. |
| Repository and module | Retain Go module identity (`tasks`) and GitHub repository coordinates (`sebastienferry/taskflow`). Working branch remains `47-rename-taskflow-to-something-else`. |
...
1. Inventory occurrences using `rg -i 'taskflow'` to classify occurrences between user-facing presentation (to rename to Sectile) and machine/protocol contracts (to retain for compatibility).
...
- **Introducing dual writable storage namespaces (`sectile/` alongside `taskflow/`)**: Rejected because dual stores risk split-brain state, synchronization bugs, and migration complexity.
...
- **Verification**:
  ...
  - Verify that `openspec validate 47-rename-taskflow-to-sectile --strict` passes.
