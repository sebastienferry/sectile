# Rename Sectile to Sectile

## Why
The product owner selected **Sectile** (slug: `sectile`) because the previous product name "sectile" is already taken and overloaded in the software ecosystem. The name *Sectile* derives from *Opus Sectile*—the Roman art of precisely cutting and fitting distinct pieces together into a seamless mosaic—fittingly capturing modular agent orchestration and task composition. This change brings consistent product presentation, executable names, and release distribution while preserving full backwards compatibility for existing installations, databases, and automation.

Source: [issue #47](https://github.com/sebastienferry/sectile/issues/47) and [settled clarification](../../../docs/clarifications/47.md).
...
- Preserve existing database discovery order (`DB_PATH`, working-directory `tasks.db`, user-config `sectile/tasks.db`, then `taskacao/tasks.db`).
- Preserve all existing configuration file paths (`.taskflow/`), environment variables (`SECTILE_*`, `TASKACAO_*`), localStorage preference keys, machine-facing health checks (`sectile-api`), and terminal execution protocol markers (`__SECTILE_*`).
...
- Renaming the remote GitHub repository (`sebastienferry/sectile`) or Go module (`tasks`).
- Relocating or migrating existing user SQLite databases (`tasks.db`).
- Modifying machine protocol markers (`__SECTILE_*`) or health check service IDs (`sectile-api`).
