...
- [x] T1.1 `models.NormalizeSpecArtifacts`; `SpecArtifacts` on
  `models.Project`, the create request and (`*string`) the update request.
  Unit test for the normaliser.
- [x] T1.2 New numbered migration adding `projects.spec_artifacts TEXT NOT
...
- [x] T1.3 Create/update validation in `db.go` ("specArtifacts must be keep or
  ...
  Not run on this workstation: no `SECTILE_TEST_POSTGRES_DSN` was set, so
  the PostgreSQL variant skipped; the SQLite variant passed.
- [x] T1.4 `agentconfig.Config.SpecArtifacts` filled by `DB.AgentConfig`;
...
- [x] T2.1 `Overrides.SpecArtifacts`; `ApplyOverrides` applies it; merge in
...
- [x] T2.2 Desktop save input `specArtifacts` / `inheritSpecArtifacts` in
...
- [x] T3.1 `internal/agent/specexclude.go`: `excludeFilePath` (shared with
  `excludeTaskWorktrees`), `artefactPatterns`, `ensureSpecExclusions`,
  `removeSpecExclusions`, atomic write.
- [x] T3.2 Tests on a temporary repository with a linked worktree: block
  ...
  stays tracked.
- [x] T3.3 Wire into `prepareDispatchLocked` (drop -> ensure, keep -> remove,
...
- [x] T4.1 Update the clarify, specify (three variants), implement and
  adjust fragments per `plan.md` decision 4; define `<n>` as the key without
  `#`.
- [x] T4.2 Launch notice in `internal/agent/agent.go` for the six skills
  when `config.SpecArtifacts == "drop"`. Test on the built prompt for
  `drop`, `keep`, and a skill outside the list.
- [x] T4.3 `UPDATE_GOLDEN=1 go test ./internal/skills/`, review the golden
...
- [x] T5.1 Agent operation `spec_artifacts` in `executeOperation`, returning
  the effective value; `localInspections` entry. Agent test for both values.
- [x] T5.2 `validateStagePR`: specify + owner specify + no `prUrl` -> ask the
...
- [x] T5.3 The `specified` policy sentence in `projectskills.go`; update the
  policy test in `internal/db/pr_policy_test.go`.

## 6. Tracked warning (FR10, US5)

- [x] T6.1 `specArtifactsTracked` in the desktop project GET, counted from
...
- [x] T7.1 `web/src/types/index.ts` and `ProjectModal.tsx`: checkbox and help
...
- [x] T7.2 `desktop/src/main.js`: "Specifications" row per decision 9, reset,
...
- [x] T8.1 `CHANGELOG.md`, `[Unreleased]` / `Added`: "Projects can keep
...
- [x] T8.2 Mention the option in the relevant `docs/` page on project
  settings if one describes `useWorktrees`; otherwise none.
- [x] T8.3 Full run: `go build ./...`, `go vet ./...`, `go test ./...`
