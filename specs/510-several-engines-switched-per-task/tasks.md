...
- [x] T1.3 `ValidateEngines`, moving the custom-provider `{prompt}` rule from
  the desktop handlers into it; messages name the engine.
- [x] T1.4 `engines_test.go`: every accessor fallback, pruning, validation
...
- [x] T2.1 `engines_migration.go`: `resolveLegacyEngine` (the current body of
...
- [x] T2.2 `ReadSettings` applies `convertEngines` after the legacy fold,
  whatever the layout.
- [x] T2.3 `MigrateSettings(legacyRoot)`: under `settingsMu`, backup
...
- [x] T2.4 `engines_migration_test.go`: one test per source shape listed in
...
- [x] T3.1 `Resolve` resolves the project default engine through a shared
  `resolve(c, s, Engine)`; new `ResolveTask(c, s, taskID)`. `Config` gains
  `EngineID` and `OnProjectDefaultEngine` (`json:"-"`).
- [x] T3.2 Setup providers extended with every catalogue provider that
  installs skills, deduplicated, configured order first.
- [x] T3.3 `ValidateExecution` / `ValidateDefaults` / `ValidateProject` refuse
...
- [x] T4.1 `ApplyWorkstationSeed` writes a catalogue entry and default on an
  ...
  its engine part.
- [x] T4.2 `seed_test.go` updated for both seeds, including the "catalogue
  already stated" case.

## 5. Task engine at dispatch (FR-5, FR-7, US4)

- [x] T5.1 Switch every task-holding `Resolve` call to `ResolveTask`
...
- [x] T5.2 `LaunchModel` ignores the one-off model off the project default
  engine, with one log line; headless refusal messages name the engine.
- [x] T5.3 Tests: override ignored and applied; `dispatchCommand` and
...
- [x] T6.1 `GET`/`PUT /desktop/engines`, `GET`/`PUT /desktop/task-engines`
  as in plan section 6; `reportCapabilitiesLater` after a catalogue save.
- [x] T6.2 `/desktop/workstation` and `/desktop/project`: engine fields
  ...
  in the project GET.
- [x] T6.3 `task-engines` in the `/desktop/status` capabilities.
- [x] T6.4 Handler tests for each endpoint (validation, 404 unknown engine,
...
- [x] T7.1 IPC handlers and preload entries `engines`, `saveEngines`,
  `taskEngines`, `setTaskEngine`, gated on the `task-engines` capability.
- [x] T7.2 `desktop/src/engines.mjs` with `nextEngine`, `taskEngine`,
  `engineMark`, `engineTooltip`, and `desktop/tests/engines.test.mjs`.
- [x] T7.3 Workstation settings: Engines section and engine editor dialog
  ...
  small hunks.
- [x] T7.4 Project dialog: "Default engine" selector replacing the engine
  rows; `execution-fields.mjs` and its tests updated;
  `project-settings.test.mjs` updated.

## 8. Desktop: ticket table toggle (US3)

- [x] T8.1 `engine` column in `TICKET_COLUMNS` when the capability is
...
- [x] T9.4 `desktop/README.md` if it describes the engine fields of the
  project or workstation settings.

## 10. Gates

- [x] T10.1 `go build ./...`, `go vet ./...`, `go test ./internal/agentconfig/...
  ./internal/agent/...` (rerun the keepalive test alone if it flakes under
  load).
- [x] T10.2 Desktop: `node --test desktop/tests/*.test.*`, the UI tests
...
- T2/T3: the flag is `OffProjectDefaultEngine` (true off the project default
  engine) rather than `OnProjectDefaultEngine`, so a configuration built by
  hand keeps applying the one-off model. The implicit engine of a file stating
  none has an identity of its own (`implicitEngineID`), which the seed uses to
  tell "nothing stated" from "`agy` stated".
- T4: a project that already picks an engine is completed by the seed (a new
  or reused entry), never edited in place.
- T6: `PUT /desktop/task-engines` stores no choice when the target is the
  project default engine. `desktopTasks` has no custom-provider check; the
  audited task-holding call sites are `prepareDispatchLocked`,
  `admitProjectRun` and `runOperation` (see the commit body).
- T7.3: the engine editor is inline in the Engines section, and every change is
  saved at once, so the default always names a stored engine.
- T9.1: the ADR is 0033, `main` took 0032 meanwhile.
- T10.3: the conversion was checked on a copy of a real pre-#305 settings file
  (credentials removed, `HOME` in `$TMPDIR`): eight projects resolved the same
  provider, templates and models before and after, the backup held the
  previous bytes, a second start changed nothing. Launching a switched task
  from the web against a live server is left to the owner.
