...
- [ ] **T1** In `internal/agentconfig/local.go`, extend `Overrides` with `Terminals map[string]string` (`json:"terminals,omitempty"`) for per-project terminal emulator overrides.
- [ ] **T2** In `internal/agentconfig/local.go`, update `ApplyOverrides(c Config, overrides Overrides) Config` to check `overrides.Terminals[c.ProjectID]`; if non-empty, set `c.ExternalTerminalCommand`.
- [ ] **T3** In `internal/agentconfig/settings.go`, include `"terminals"` in `WriteSettings` for atomic persistence to `~/.config/sectile/settings.json`.
- [ ] **T4** In `internal/agentconfig/local_test.go` and `internal/agentconfig/settings_test.go`, add unit tests validating `Terminals` precedence, fallback to workstation `Terminal`, and settings file round-trip serialization.

---

## 2. Agent Terminal Attach Client (`cmd/agent`, `internal/agentattach`)

- [ ] **T5** Create `internal/agentattach/attach.go`:
...
- [ ] **T6** In `cmd/agent/main.go`, register the `attach` subcommand:
  ```go
  case "attach":
      ...
      return
  ```
- [ ] **T7** In `internal/agentattach/attach_test.go`, add unit tests:
...
- [ ] **T8** In `internal/agent/terminal_launcher.go`:
...
- [ ] **T9** In `internal/agent/terminal_launcher_test.go`, add unit tests asserting command construction for supported terminal emulators.
- [ ] **T10** In `internal/agent/agent_desktop.go`:
...
- [ ] **T11** In `internal/agent/agent_desktop_test.go`, add tests for `POST /desktop/terminal/detach` and `POST /desktop/tasks/terminal-external`.

---

## 4. Electron Desktop Companion Bridge (`desktop/electron/`)

- [ ] **T12** In `desktop/electron/preload.cjs`, expose:
  - `launchNativeDiscussion(projectId, taskId, terminal)`
  - `detachToNativeTerminal(runId, terminal)`
- [ ] **T13** In `desktop/electron/main.cjs`, implement IPC handlers:
...
- [ ] **T14** In `desktop/src/main.js` (Ticket row menu `renderTicketRow`):
  - Add **Discussion in native terminal** entry to the `…` dropdown menu for open tasks.
  - On click, call `api.launchNativeDiscussion(...)`.
- [ ] **T15** In `desktop/src/main.js` (Console toolbar and execution view):
...
- [ ] **T16** In `desktop/src/main.js` (`openProject` dialog → Local tab):
...
- [ ] **T17** In `desktop/src/style.css`, add styles for the native terminal status badge and toolbar button.

---

## 6. Quality Gates & Verification

- [ ] **T18** Run Go test suite: `go test -v -race ./internal/agentconfig/... ./internal/agent/... ./internal/agentattach/...`.
- [ ] **T19** Run Desktop unit tests: `npm test` inside `desktop/`.
- [ ] **T20** Verify all acceptance criteria in `spec.md` (US1 through US5).
