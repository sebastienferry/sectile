...
Review refinement: existing Scaffold retires every absent manifest entry, including other providers. Explicit initialization uses a provider-scoped scaffold variant that retains other providers' manifest entries while still retiring removed skills for the selected provider. General task preparation keeps its existing behavior. Regression coverage initializes Codex, Claude and an MCP-only provider sequentially, then removes a Claude skill and verifies Codex remains installed.
