...
- [ ] T1.1 `Binding.Server bool`, `ServerBinding(tracker)`, `bytes()` with the
  `sectile:v1:server:tracker:` prefix; `Seal` accepts an empty user when
  `Server` is set.
- [ ] T1.2 Tests: a server record opens with `ServerBinding` of its tracker
...
- [ ] T3.2 `trackerCredentials` fills tokens and Jira e-mail from the new store
...
- [ ] T3.3 `withoutTrackerTokens` computes `…Set` / `…FromEnv` from the states;
  `jiraEmail` blanked for a member answer (handler side passes the role).
- [ ] T3.4 `UpdateSettings` and `SaveTrackerCredentials` stop writing the token
  and Jira e-mail columns.
- [ ] T3.5 Tests: database wins over environment; Jira row never mixed with
...
- [ ] T4.1 `NewClient` reads only `SECTILE_GITHUB_TOKEN`,
...
- [ ] T4.2 Rewrite `client_test.go` / `jira_test.go` cases that relied on
...
- [ ] T4.3 Startup warning in `cmd/server/main.go` from
...
- [ ] T5.1 `SkillJob.RequestedBy`; `EnqueueSyncWith` fills it and leaves
  `ActingUser` empty for sync jobs; activity `UserID` unchanged.
- [ ] T5.2 `processSyncJob` drops `tracker.WithActingUser`; any display use of
  `job.ActingUser` moves to `RequestedBy`.
- [ ] T5.3 `autosync.go` enqueues with `""`; remove the owner comment.
- [ ] T5.4 `missingServerCredential` and the split at the call sites
  (`client.go`, `github.go`, `jira_client.go` incl. the missing e-mail case);
  unreadable-credential message.
- [ ] T5.5 Tests (`internal/db`, fake tracker HTTP server recording the
...
- [ ] T6.1 `adoptLegacyServerTrackerTokens`, called after migrations in the
  open path; transaction per provider; `ON CONFLICT (tracker) DO NOTHING`.
- [ ] T6.2 `internal/db/migrate.go`: copy `server_tracker_credentials`;
  `ensureKeyOpensCredentials` also tries one server record.
- [ ] T6.3 Tests: clear-text GitHub and Jira (with e-mail) in `settings` →
...
- [ ] T7.1 `internal/handlers/servercredentials.go`: `GET`, `PUT`, `DELETE`,
...
- [ ] T7.2 `trackerSettingsKeys` without the four credential keys; settings
  update ignores them for everyone.
- [ ] T7.3 `HandleTrackerSetup`: token or e-mail from a member → 403 with a
  ...
  member.
- [ ] T7.4 Tests: anonymous 401 and member 403 on each admin route; member
...
- [ ] T8.4 `npx tsc --noEmit`, oxlint and `npm test` in `web/` (symlink the
  main checkout's `node_modules` if the worktree has none, remove it after).

## 9. Documentation

- [ ] T9.1 `docs/adrs/0028-tracker-sync-uses-a-server-credential-per-provider.md`
  (check the next free number on `origin/main`); "Superseded in part by ADR
  0028" line in ADR 0014.
- [ ] T9.2 `README.md` (tracker connection section and env table),
...
- [ ] T9.3 `CHANGELOG.md` `[Unreleased]`: `Changed` (syncs use the provider's
