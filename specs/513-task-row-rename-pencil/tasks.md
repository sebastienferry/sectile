...
- [ ] T1.1 `desktop/src/main.js`: `displayedName(run)` helper, used by the
  row title and the new pencil label.
- [ ] T1.2 `renaming` state (`{key,draft,fresh}`), and in `render()` the
...
- [ ] T1.3 `finishRename(run,commit)`: trimmed, non-empty, changed value
...
- [ ] T1.4 Pencil button `.task-rename-button` after the archive button, on
  every row, `Rename <name>` label and tooltip, line SVG icon; it does not
  select the run.

## 2. Remove the "…" menu (FR-1, FR-10) - `feat(desktop)`

- [ ] T2.1 Delete the `.task-menu` button and `taskMenu(run)`; keep
  `requestArchive` and `archiveTask`.
- [ ] T2.2 `desktop/src/style.css`: pencil visibility shares the
...
- [ ] T3.1 `console.ui.cjs`: rename through the pencil and Enter; drop the
  dialog's Detach check (the toolbar check stays).
- [ ] T3.2 `task-header.ui.cjs`: rename through the pencil; tracker title
  change and restart checks unchanged.
- [ ] T3.3 `pr-display.ui.cjs`: `.task-menu` → `.task-rename-button`.
- [ ] T3.4 Check that the `Relaunch` clicks of `console.ui.cjs`,
  `free-console.ui.cjs` and `skill-mode.ui.cjs` still resolve to the toolbar.
- [ ] T3.5 New `desktop/tests/task-rename.ui.cjs` covering the ten cases of
...
- [ ] T3.6 `grep -rn "task-menu\|taskMenu\|Actions for #" desktop/` returns
  nothing but project and ticket menus.
- [ ] T3.7 `cd desktop && npx vite build && npm test && npm run test:ui`
