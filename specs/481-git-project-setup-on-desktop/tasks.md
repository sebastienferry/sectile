...
- [x] T1 Add `internal/agent/agent_desktop_git_init.go` with
      `gitFolderState(ctx, path) (top, state string)` returning `ready`,
      `unborn`, `folder` or `missing` (plan, "Agent endpoint").
- [x] T2 Add the `GET /desktop/git-init?path=` handler returning
      `{path, state}`.
- [x] T3 Add the `POST /desktop/git-init` handler under `d.prepareMu`:
...
- [x] T4 Route `/desktop/git-init` in `agent_desktop.go` and add `git-init` to
      the `/desktop/status` capabilities.
- [x] T5 Tests in `internal/agent/agent_desktop_git_init_test.go`, with a
...
- [x] T6 `desktop/electron/main.cjs`: `git-state` (answers `{state:'unknown'}`
      ...
      lacks it).
- [x] T7 `desktop/electron/preload.cjs`: expose `gitState` and `gitInit`.

## 3. Renderer

- [x] T8 `desktop/src/git-init.mjs`: `offerFor(state)` and the texts of plan,
      "Renderer"; unit tests in `desktop/tests/git-init.test.mjs` (every state,
      the explanation's four points).
- [x] T9 `desktop/src/main.js`: `attachGitOffer` under the Local repository
      row; examined at open, after *Browse*, on `change`; stale answers
      discarded; "Initialize" and "Not now" per spec US1 and US4.
- [x] T10 Attach the same offer to the Specifications folder input, examined
      only while it is shown; stored plain folder examined at open (US2);
      `onReady` refreshes `renderSpec` with the new value.
- [x] T11 `desktop/src/style.css`: offer block, reusing the existing notice and
      button styles.
- [x] T12 `desktop/tests/git-init.ui.cjs` (fake agent modelled on
      `spec-folder.ui.cjs`): the five scenarios of plan, "Test strategy" /
      Playwright. Run after `npx vite build`.

## 4. Documentation

- [x] T13 `desktop/README.md`: the offer, the empty first commit and its
      consequence on worktrees, under Local repository and Specifications
      folder.
- [x] T14 `CHANGELOG.md`: the `[Unreleased]` / `Added` line of plan,
      "Documentation".

## 5. Checks before the implemented transition

- [x] T15 `go test ./internal/agent/...` (with `-race` as CI does); `go vet`.
- [x] T16 `cd desktop && npm test`, then `npx vite build` and the new UI test
...
- [x] T18 Restore `web/webui/.gitkeep` if a build removed it; the checkout must
