...
- [x] T1.1 Add `shared/sidebarShortcut.mjs` and `shared/sidebarShortcut.d.mts`:
      `isMacPlatform`, `sidebarShortcutAction`, `sidebarShortcutLabel`,
      `sidebarShortcutAria`.
- [x] T1.2 Add `web/tests/sidebarShortcut.test.mjs`: the decision table of
      `plan.md`, `isMacPlatform` on both navigator shapes, both labels.

## 2. Web (US1, US2, US3, US4, US5.1, US5.3, FR5-FR8)

- [x] T2.1 Persist `sidebarCollapsed` under `sectile_sidebar_collapsed` in
      `AppContext.tsx`, both reads and writes inside `try`.
- [x] T2.2 Add the chord branch to the global keyboard handler, before the
      `isInputActive` checks; add `isCloneModalOpen` to the modal test and the
      effect dependencies.
- [x] T2.3 Tooltips and `aria-keyshortcuts` on both toggle buttons in
      `Sidebar.tsx`.
- [x] T2.4 `toggle_sidebar` command in `CommandPalette.tsx`.

## 3. Desktop (US1.3-US1.5, US2.1-US2.3, US3.2, US5.2, FR5)

- [x] T3.1 Extract `toggleSidebar()` from the `#toggle-sidebar` handler.
- [x] T3.2 Capture-phase chord listener beside the Cmd/Ctrl+K one.
- [x] T3.3 Tooltip and `aria-keyshortcuts` in `renderSidebarToggle`.

## 4. Changelog (FR9)

- [x] T4.1 One line under `[Unreleased]` / `### Added` in `CHANGELOG.md`,
      naming both clients and the web persistence, with `(#474)`.

## 5. Verification

- [x] T5.1 Add `web/tests/sidebar-shortcut.browser.mjs`.
- [x] T5.2 Add `desktop/tests/sidebar-shortcut.ui.cjs`.
- [x] T5.3 `npm run build`, `npm run lint`, `npm test` in `web/`; run
      `sidebar-shortcut.browser.mjs` and `board-views.browser.mjs`.
- [x] T5.4 `npm test` in `desktop/`; `npx vite build`, then run
...
- [ ] T5.5 By hand: Ctrl+B in Firefox does not open the bookmarks sidebar.
      Not done during implementation: no Firefox on the workstation, and the
      Playwright install carries Chromium only. Left to the reviewer.
- [x] T5.6 Re-read the diff.
