...
- [x] T1.1 Create `internal/sddfiles` with `SearchRoots`, `FindMacroDir`,
...
- [x] T1.2 Move the lookup tests from `internal/db/sddslicing_test.go` /
...
- [x] T2.1 `agentconfig.Config.MonoRepo *bool`, filled by `DB.AgentConfig`;
  test in `internal/db/agentconfig_test.go` and the projection test in
  `internal/handlers/mcp_test.go`.
- [x] T2.2 `localSpecRepo(overrides, projectID, root, monoRepo)`; callers in
...
- [x] T3.1 `ensureMacroWorktree`: plain folder returns the folder, empty
...
- [x] T3.2 Launch environment with an empty branch: `SECTILE_SPEC_BRANCH=""`,
  `SECTILE_SPEC_WORKTREE=false`; check `dispatchCommand` with an empty
  branch.
- [x] T3.3 `realign_macro` fragments: empty-branch rule (write in `path`, no
...
- [x] T4.1 `agentprotocol.Operation.SpecFile`; agent `macro_spec_file` routed in
  ...
  temporary folder.
- [x] T4.2 `"macro_spec_file": 15s` in `localInspections`; extend
  `agentoperations_test.go`.
- [x] T4.3 `DB.TodosFromSDD(ctx, userID, projectID, key, source)` reads through
...
- [x] T4.4 Slicing handler passes `r.Context()` and `h.webSessionUser(r)` and
...
- [x] T5.1 New numbered migration dropping `projects.spec_repo_path`; never the
  baseline. Adjust `migrations_test.go` and `activerun_test.go`, which drop the
  column to simulate old schemas.
- [x] T5.2 Remove `SpecRepoPath` from `models.Project`,
...
- [x] T5.3 Web: remove the "Dépôt des spécifications" field, its state, payload
...
- [x] T6.1 Map-project handler: absolute + existing directory required; Git
...
- [x] T6.2 Project info `GET`: `specDefault`, `specKind`
  (`git`/`folder`/`missing`/`unset`). Tests for each kind.
- [x] T6.3 `desktop/src/main.js`: label "Specifications folder"; placeholder
...
- [x] T7.1 `docs/contracts/server-agent-v1.md`: `macro_spec_file`, `monoRepo`
  in the config, empty `branch` in `macro_worktree`, empty
  `SECTILE_SPEC_BRANCH`.
- [x] T7.2 ADR `0027-specifications-folder-is-a-workstation-setting.md`;
  "superseded in part" note in ADR 0026.
- [x] T7.3 `prepare_macro_worktree` tool description in
  `internal/taskmcp/server.go`.
- [x] T7.4 `CHANGELOG.md` `[Unreleased]`: `Changed` and `Removed` lines (FR10).

## Implementation notes

- T7.4: the `[Unreleased]` line #426 added for the web "Dépôt des
  spécifications" never shipped in a release, so it is rewritten to describe
  the final behaviour rather than paired with a `Removed` line announcing the
  removal of an option no release carried. FR10's intent (the change is
  recorded under `[Unreleased]`) is kept.
- Two pre-existing workstation settings defects blocked US1-US3 and were fixed
  in group 6: `localProjectRoot` never merged `specRepos` into the overrides the
  macro operations read, and `WriteSettings` kept the file's `specRepos` when
  the last folder was cleared.
- `desktop/README.md` documents the setting; the root `README.md` MCP tool line
  mentions it.
