...
- [x] **T0** Get an explicit answer on **OPEN-1** (authoritative GitHub mechanism),
...
- [x] **T1** In `internal/trackerapi/github.go`: add `CreatedAt time.Time` to
  `PullRequest` and populate it in `BranchPullRequest` so ordering is uniform.
- [x] **T2 (OPEN-1)** In `internal/trackerapi/github.go`: add
...
- [x] **T3** In `internal/trackerapi/github_test.go`: `httptest` coverage of T2 —
...
- [x] **T4** In `internal/tracker/ticketing.go`: declare the optional
  `PullRequestDiscoverer` interface. Do **not** implement it on
  `BaseTicketingSystem`.
- [x] **T5** In `internal/trackerapi/adapters.go`: add the discovery capability to
  the capability set, announce it on `GithubAdapter` only, and implement
  `GithubAdapter.IssuePullRequests` by delegating to T2.
- [x] **T6** In `internal/trackerapi/adapters_test.go`: assert GitHub announces the
...
- [x] **T7** Create `internal/db/prdiscovery.go` with the injectable hook
  `d.prDiscoveryLookup`, mirroring `d.prEvidenceLookup`.
- [x] **T8 (OPEN-3)** Implement `discoverTaskPullRequests`: evaluate the gate
...
- [x] **T9 (OPEN-2, OPEN-4)** Implement `applyDiscoveredPullRequests`:
...
- [x] **T10 (OPEN-2)** If the answer is "remember the detachment": add the
  ...
  passes.
- [x] **T11** Create `internal/db/prdiscovery_test.go` covering: empty-set seeding
...
- [x] **T12** In `internal/db/db.go`: call discovery from `SyncSingleTaskAs`
...
- [x] **T13 (OPEN-3)** In `internal/db/autosync.go`: distinguish the full-sync pass
...
- [x] **T14** Report on the sync activity: one step per task with the attached URLs,
...
- [x] **T15** Rate limiting: a 429 on discovery enters the existing auto-sync
...
- [x] **T16** Extend the fake tracker of `internal/db/jiratracker_test.go:70` and
...
- [x] **T17** Expose the `force` flag on the single-task sync path so a
...
- [x] **T19 (OPEN-2)** If T10 added a column, extend
  `internal/db/postgres_writepaths_test.go` accordingly.
- [x] **T20** Run `make test` (Go, `web` node tests, `tsc --noEmit`, `oxlint`) and
  fix every failure.
- [x] **T21** Author `docs/adrs/0017-pull-requests-are-rediscovered-on-sync.md`
  recording the issue-anchored discovery choice and the rejected alternatives of
  `plan.md`.
- [x] **T22** Update `docs/CAPABILITIES.md` (and `docs/API_AND_DATA_SPEC.md` if the
...
- **T17 / T18** — no front-end change was needed: the task detail view already
  triggers `POST /api/tasks/{id}/sync`, which now goes through
  `ForceSyncSingleTask` and bypasses the bounding rule. T18 is therefore not
  applicable.
- **T13** — the full-sync / incremental distinction needed no change in
  `autosync.go`: the incremental pass re-reads tickets through
  `SyncSingleTaskAs`, which never discovers, and discovery hangs off
  `afterTrackerSync`, which only a full synchronisation reaches. The
  distinction is structural rather than a flag.
- **T19** — `TestPostgresPullRequestDetachmentFlag` covers the new column. Like
  every PostgreSQL test it skips unless `SECTILE_TEST_POSTGRES_DSN` is set;
  parity itself is covered by `TestSchemaIsCompleteWithoutLegacyMigrations`.
- **T23** — not applicable: the repository has no `CHANGELOG.md`.
