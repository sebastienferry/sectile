...
1. [ ] Create temporary or initial test in `internal/db/skills_test.go` to generate golden files in `internal/db/testdata/golden/` from the current unrefactored renderer for all 10 skills across `speckit` and `openspec`.
2. [ ] Verify that all golden files (`*.skill.md` and `*.command.md`) are cleanly generated and version-controlled.

## Phase 2: Markdown Fragments Extraction
3. [ ] Create `internal/db/skills/contracts/` and extract shared contracts: `task-access.md`, `session-title.md`, and `transition.md`.
4. [ ] Extract fragments for stage skills:
...
5. [ ] Extract fragments for utility and composite skills:
...
6. [ ] Mount `//go:embed skills/*` in `internal/db/`.
7. [ ] Remove prose fields (`goal`, `readFirst`, `stepsBody`, `guard`, `report`) from `StageSkill` struct in `internal/db/skilltemplates.go`.
8. [ ] Implement fragment loading, template substitution, and composite assembly in `RenderSkillContent` and `RenderSkillCommand`.

## Phase 4: Validation & CI Checks
9. [ ] Run golden file regression tests asserting 100% byte-for-byte identity across all skills and framework variants.
10. [ ] Add automated CI checks in `skills_test.go` verifying YAML frontmatter validity and fragment completeness.
11. [ ] Run `make test` and full repository checks to verify zero regression.
