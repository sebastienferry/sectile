...
- [x] T1.1 In `DB.CreateTaskAs`, resolve `TrackerForProject` once, before the
...
- [x] T1.2 Forward `IssueType` and `ParentKey` (trimmed) in
  `tracker.CreateIssueRequest`. Leave `Assignee` and `Sprint` out.
- [x] T1.3 Wrap tracker errors as `<Tracker> issue creation failed: %w`, with
  `trackerTitle` for `github` and `jira`, in both the error and the
  empty-response branches.
- [x] T1.4 Tests in `internal/db`: rework `quick_add_test.go` so its comment
...
- [x] T3.1 `create_task`: build `ctx := tracker.WithActingUser(ctx, callerOf(resolve, req).UserID)`
  and call `database.CreateTaskAs(ctx, …)`.
- [x] T3.2 `get_task`: `database.GetTaskCommentsAs` with the same context.
- [x] T3.3 Rework `TestCreateTaskFailsRatherThanFilingLocally`: the refused
  project becomes one whose tracker cannot create (`gitlab`); the Jira case
  moves to the new tests below.
- [x] T3.4 New test: Jira-backed project against an `httptest` Jira site
...
- [x] T3.5 New test with `NewServerWithCallers` and a resolver naming a user:
...
- [x] T3.6 New test: `get_task` with a named caller reads comments with the
  caller's token; without a personal token it returns the task and a
  `commentsError`.
- [x] T3.7 The existing GitHub `create_task` test passes with its assertions
  unchanged.

## 4. Documentation and verification (FR9)

- [x] T4.1 `CHANGELOG.md`, `[Unreleased]` › `Fixed`: agents can file tickets on
...
- [x] T4.2 `go vet ./...` and `go test ./internal/...` green; a failure of the
