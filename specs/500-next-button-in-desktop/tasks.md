...
- [ ] T2.1 `submittingSteps` becomes a `Map` key -> skill id; update its
  `add`/`delete`/`has` call sites in `desktop/src/main.js` (plan decision 2).
- [ ] T2.2 `renderNextStep()`: resolve `current` from the most recent active
...
- [ ] T2.3 Extend `desktop/tests/next-step.ui.cjs`:
  ...
  - optional: an active `discuss` run on a `#finished` task shows
    `Current: Discuss` (FR7).
- [ ] T2.4 Update any lookup in the other `*.ui.cjs` files that searches the
  button by `Next: <label>` while a run is active.

## 3. Changelog (FR9)

- [ ] T3.1 Add under `## [Unreleased]` a `### Changed` section (create it
...
- [ ] T4.3 Manual check in the desktop app against a throwaway server (never
