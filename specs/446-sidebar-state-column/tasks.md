...
1. [x] Rebase `feat/446` on `origin/main` and check nothing on `main` already
       reorders the sidebar row.
2. [x] `desktop/tests/sidebar-alignment.ui.cjs`: write the alignment test first
...
3. [x] `desktop/src/main.js`: move `span.run-state` out of `button.run` as the
...
4. [x] `desktop/src/style.css`: glyph left padding, `.task-key-slot` (`5ch`
       min-width, no shrink, nowrap), tabular figures on the key, retarget the
       `.local-task>.task-number` rules.
5. [x] Grep the UI tests for selectors assuming the glyph is inside `.run` and
       adjust them without weakening their assertions.
6. [x] `CHANGELOG.md`: one line under `## [Unreleased]` → `### Changed`
       (`(#446)`).
7. [x] Gate: desktop `npm test`; `npx vite build` then the desktop UI tests
...
8. [x] Visual check in the running Desktop app: glyphs and titles in columns
