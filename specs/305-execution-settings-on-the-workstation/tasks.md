...
- [ ] T1.1 `internal/agentconfig/workstation.go`: `Execution`, `Defaults`,
...
- [ ] T1.2 `ReadSettings`: decode the legacy keys into `legacySettings` and
  `foldLegacy` them when `layout` is absent; `.taskflow/agent.json` fallback
  through the same fold, rules unchanged.
- [ ] T1.3 `WriteSettings`: delete every legacy key, write `layout: 2`, null an
  emptied `projectSettings` / `repositories` / inner map, keep connection
  fields, atomic 0600.
- [ ] T1.4 Tests: every row of the fold table; legacy file read then written
...
- [ ] T2.1 `Resolve(c, s)` replacing `ApplyOverrides` (plan §2 steps 1-8),
  `ExecutionLimit` reading project then defaults; `EffectiveCommandTemplate`
  applied on the agent.
- [ ] T2.2 `DefaultProviderModels` and `ProviderModels(defaults, provider)`
  (list moved from `web/src/lib/aiModels.ts`).
- [ ] T2.3 Replace every `ApplyOverrides` call site and every
...
- [ ] T2.4 Tests ported from `local_test.go` / `model_test.go`: provider
...
- [ ] T5.1 Migration 23 `agent_capabilities` (plan §6), migration 24
  ...
  migrations.go*).
- [ ] T5.2 Remove `TtyMode` from models, project `SELECT`/`INSERT`/`UPDATE`,
  web types.
- [ ] T5.3 Update the forget-and-replay fixtures and the rewind helpers for
  both migrations; migration tests on SQLite and PostgreSQL (throwaway DSN
  only).

## 6. Seed (FR-8, US6)

- [ ] T6.1 Handler `GET /api/v1/agent/execution-seed` (plan §4), route in
  `cmd/server/main.go`, caller's `UserSettings` for terminal and editor.
- [ ] T6.2 `internal/agent/seed.go`: defaults seed on first connection,
...
- [ ] T6.3 Tests: handler auth and content; agent seed cases of plan *Test
...
- [ ] T7.1 `db.SaveCapabilities`, `db.EngineReport` (presence and fallback
  slots, live instances only), in new `internal/db/capabilities.go`.
- [ ] T7.2 Handlers `PUT /api/v1/agent/capabilities`,
  `GET /api/projects/{id}/engine`.
- [ ] T7.3 Agent `capabilityOf(config)` and `reportCapabilities` after
  registration and after each desktop save.
- [ ] T7.4 `ResolveTaskEngine(projectID, userID, deviceID, skillID, model)`
  from the report, unknown without; update the three callers.
- [ ] T7.5 Tests: upsert/read, other user's report never served, dead
...
- [ ] T8.1 `sessionContext` without `useWorktrees`, `aiProvider`, `aiModel`;
  tests updated.

## 9. Full-chain refusal (FR-11, US8)

- [ ] T9.1 `chain_test.go`: fake agent answering `execute_skill` with the
...
- [ ] T9.2 If the reason is lost or replaced on the way (dispatcher,
...
- [ ] T10.1 Agent routes `GET/PUT /desktop/workstation`; `mapProject` new
  fields and validation; `desktopProject` answer with `{value, inherited,
  source}`; capability report after each save.
- [ ] T10.2 Electron: `save-settings` keeps connection keys only; IPC
  `workstation-settings` / `save-workstation-settings` in `main.cjs` and
  `preload.cjs`.
- [ ] T10.3 `desktop/src/main.js`: "Execution defaults" panel (every US4
...
- [ ] T10.4 Tests: agent route validation (Go); desktop UI tests after
  `npx vite build` (restore `webui/.gitkeep` afterwards, memory).

## 11. Web (FR-4, US3, US5)

- [ ] T11.1 `ProfileModal`, `ProjectModal` (`agent` category removed,
...
- [ ] T11.2 `useProjectEngine`, `TaskCard` and `TaskDetailModal` on the
  report; unknown badge translations (fr/en).
- [ ] T11.3 Clean `lib/aiModels.ts`, delete unused `AIModelField`,
  `ProviderModelsField`, `lib/projectAgentSettings.ts`; `types/index.ts`.
- [ ] T11.4 Tests (`node --test`, component tests; worktree has no
