...
- [ ] **T1** Create `desktop/assets/icon.svg`:
...
- [ ] **T4** Generate `desktop/assets/icon.ico`:
...
- [ ] **T5** In `desktop/electron/package.cjs`:
...
- [ ] **T6** In `desktop/electron/main.cjs` (`openWindow`):
  - Pass `icon: path.join(__dirname, '../assets/icon.png')` into `new BrowserWindow({ ... })`.
- [ ] **T7** In `desktop/electron/main.cjs` (application initialization):
...
- [ ] **T9** In `desktop/tests/icon.test.cjs`:
...
- [ ] **T10** Run full desktop test suite:
  ```bash
  cd desktop && npm test
  ```
- [ ] **T11** Verify acceptance criteria in `spec.md` (US1 through US5).
