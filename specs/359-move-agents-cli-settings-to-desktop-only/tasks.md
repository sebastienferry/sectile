...
- [ ] **T1** In `internal/db/db.go`, remove the server-side `models.SupportsAutonomousRun` preflight check from `EnqueueFullChainRun(taskID string)`.
- [ ] **T2** In `internal/db/`, update existing unit tests in `db_test.go` or `chain_test.go` to verify that `EnqueueFullChainRun` succeeds without requiring server-stored CLI templates.
- [ ] **T3** In `internal/agentconfig/local.go`, update `ApplyOverrides`:
...
- [ ] **T4** In `internal/agent/agent_run.go` (and/or `agent_operations.go`), add local preflight capability validation:
...
- [ ] **T5** In `internal/agentconfig/local_test.go` and `internal/agent/agent_desktop_test.go`, add unit tests:
...
- [ ] **T6** In `web/src/components/ProfileModal.tsx`:
...
- [ ] **T7** In `web/src/components/ProjectModal.tsx`:
...
- [ ] **T8** In `web/src/components/AIModelField.tsx` and related components:
  - Update any props or warnings that assumed `commandTemplate` was passed from the parent modal.
- [ ] **T9** In `web/tests/profileModal.test.mjs` and web test suite:
...
- [ ] **T10** In `desktop/electron/preload.cjs` and `desktop/electron/main.cjs`:
...
- [ ] **T12** In `desktop/src/main.js` (`openProject` dialog):
...
- [ ] **T13** In `desktop/tests/project-settings.test.mjs`:
