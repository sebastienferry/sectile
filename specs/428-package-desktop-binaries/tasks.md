...
- [x] T1.1 `desktop/electron/package-options.cjs`: pure
  ...
  platform/arch.
- [x] T1.2 `package.cjs`: parse `--platform`, `--arch`, `--agent`, `--out`;
  stage the agent as `agentName(platform)` mode `0755` in a temporary
  directory; remove it afterwards; exit 1 on a missing agent.
- [x] T1.3 `desktop/tests/package-options.test.cjs` (plan test plan).
- [x] T1.4 Check locally: `make desktop-package` output unchanged; packaging
...
- [x] T2.1 `scripts/release/build-binaries.sh`, moved verbatim from
  `build:binaries`; `.gitlab-ci.yml` `build:binaries` calls it (no
  behaviour change yet, `SHA256SUMS` still written there).
- [x] T2.2 `scripts/release/check-desktop-version.sh`.
- [x] T2.3 `scripts/release/package-desktop.sh` (mapping, archive, Linux
  agent `--version` check).
- [x] T2.4 `scripts/release/changelog-section.mjs` and
  `desktop/tests/changelog-section.test.mjs`.
- [x] T2.5 Local run of `package-desktop.sh` for the four targets; inspect
...
- [x] T3.1 `build:desktop` job, tag-only, `ELECTRON_CACHE` cache, `zip`
  installed, four targets, `dist-desktop/` artifact.
- [x] T3.2 `build:binaries` stops writing `SHA256SUMS`; `publish:binaries`
  needs `build:desktop`, merges `dist-desktop/` into `dist/`, writes
  `SHA256SUMS`, uploads.
- [x] T3.3 Header comment of `.gitlab-ci.yml`: desktop archives, tag-only,
...
- [x] T4.1 `.github/workflows/release.yml`: trigger, permissions,
  concurrency, SHA-pinned actions, jobs `binaries`, `desktop-linux`,
  `desktop-macos`, `release`.
- [x] T4.2 Create-or-update logic of the `release` job (FR13).
- [x] T4.3 Grep the workflow and `scripts/release/` for internal names (the
...
- [x] T5.1 `desktop/README.md` "Install a release" section (per-platform
  steps, `xattr` command, SmartScreen, checksums); `README.md` releases
  section links to it.
- [x] T5.2 `AGENTS.md` step 6; ADR 0003 unsigned sentence; ADR 0018
  consequence; new `docs/adrs/0034-a-release-is-published-on-both-forges.md`.
- [x] T5.3 `CHANGELOG.md` `Added` line under `[Unreleased]`.
...
Pending after implementation, since they need a pushed tag, a fork or another
platform: T3.4 (the MR pipeline of this branch), T4.4 (fork rehearsal) and
T5.4 (launching each archive). T1.4 and T2.5 were run on macOS arm64: the four
targets package, `Sectile.app` keeps its symlinks, modes are 0755 and the agent
sits at `resources/sectile-agent[.exe]`; packager downloads the Electron zips
even with `ELECTRON_SKIP_BINARY_DOWNLOAD=1`.
