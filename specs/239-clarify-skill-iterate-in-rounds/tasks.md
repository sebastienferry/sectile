...
- [x] **T1** In `internal/db/skilltemplates.go`, update `StageSkills[0]` (`ID == "clarify"`):
...
- [x] **T2** In `internal/db/skilltemplates.go`, update `renderTicketTransitionContract`:
...
- [x] **T3** In `internal/db/skilltemplates.go`, verify `renderPickupSteps`:
...
- [x] **T4** In `internal/db/skills_test.go`, extend `TestGeneratedSkillContracts`:
...
- [x] **T5** In `internal/db/skills_test.go`, add dedicated test `TestClarifySkillRoundLoopInvariants`:
...
- [x] **T6** In `docs/CAPABILITIES.md`, update `Stage 1: Clarification (clarify-issue / /clarify)`:
...
- [x] **T7** In `docs/README.md`, update section 2 (`Core Capabilities & Workflows`):
...
- [x] **T8** Run `go test -v ./internal/db/...` to verify all database and skill template tests pass cleanly.
- [x] **T9** Run `go vet ./...` to ensure no lint regressions.
- [x] **T10** Review git diff with `git diff` to ensure changes strictly match the settled scope of ticket #239.
