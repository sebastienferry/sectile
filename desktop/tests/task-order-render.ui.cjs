...
test('sidebar orders tasks across refreshes while retaining selection and history',async()=>{
 ...
 try{
  ...
  assert.deepEqual(await order(),['#4','#1','#2','#5','#3'])
  assert.equal(await page.locator('.execution-queue').count(),0)
  // The queue view is an item of each project's menu; its opener carries the waiting count.
  const alphaQueue=page.getByRole('button',{name:'Actions for Alpha',exact:true})
  const zebraQueue=page.getByRole('button',{name:'Actions for Zebra',exact:true})
  const showQueue=async opener=>{await opener.click();await page.getByRole('menuitem',{name:/^Show execution queue/}).click()}
  ...
  await expect(page.getByRole('menuitem',{name:'Show execution queue · 2 waiting',exact:true})).toBeVisible()
  await page.keyboard.press('Escape')
  await showQueue(alphaQueue)
  assert.equal(await page.locator('.local-task').count(),0)
  await expect(page.locator('.queue-summary')).toHaveText('2 active · 2 waiting')
  await showQueue(zebraQueue)
  ...
  assert.equal(await page.locator('.local-task').count(),0)
  await showQueue(alphaQueue)
  await expect(page.getByRole('region',{name:'Execution queue for Alpha'})).toBeVisible()
