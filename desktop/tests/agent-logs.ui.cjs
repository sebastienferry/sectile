...
test('agent logs work offline, render literal snapshots and recover from read failures',async()=>{
 ...
 try{
  ...
  await refresh.click();await expect(output).toHaveText(literal)
  assert.equal(await output.locator('img').count(),0)
  assert.equal(await page.evaluate(()=>window.injected),undefined)
  fs.appendFileSync(file,'Retried\n')
  await refresh.click();await expect(output).toHaveText(literal+'Retried\n')
  ...
  const bounds=await output.boundingBox(),footer=await page.locator('.dialog-footer').boundingBox()
  assert.ok(bounds.y+bounds.height<=footer.y+1,'Newest output stays above the footer')
...
test('agent logs preserve the connected execution and ignore late reads after another dialog opens',async()=>{
 ...
 const server=http.createServer((req,res)=>{
  ...
  if(req.url==='/desktop/runs'){res.end(JSON.stringify([{id:'run',taskId:'task',taskKey:'#71',projectId:'p',skill:'pickup',status:'failed'}]));return}
 ...
 try{
  ...
  const selected=await page.locator('.run.selected').textContent()
  ...
  await page.keyboard.press('Escape')
  assert.equal(await page.locator('.run.selected').textContent(),selected)
