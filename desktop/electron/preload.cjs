...
contextBridge.exposeInMainWorld('localAgent',{
 ...
 launchServerTask:(id,taskID,skillID,prompt,mode,force,viewID)=>ipcRenderer.invoke('launch-server-task',id,taskID,skillID,prompt,mode,force,viewID),
 views:()=>ipcRenderer.invoke('views'),
 viewTasks:(viewId,q,launchable=false)=>ipcRenderer.invoke('view-tasks',viewId,q,launchable),
 setViewDirectory:(viewId,path)=>ipcRenderer.invoke('set-view-directory',viewId,path),
