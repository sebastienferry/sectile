...
<section id="setup" hidden><div id="agent-offline" role="status" hidden><strong>Local agent is stopped</strong><p>Start the agent to run tasks and access your local consoles.</p></div><h1>Connect to Sectile</h1><p>In the Sectile web interface, under your profile, either create an API key and paste it here, or generate a pairing code and paste that instead. A code is single use and expires within ten minutes; it is exchanged once for a key kept on this machine.</p>
<form id="start"><label>Sectile server<input name="server" type="url" value="http://localhost:8090" required></label><label>Pairing code<input name="code" type="text" autocomplete="off" spellcheck="false" placeholder="Paste the code from the web interface"></label><label>API key<input name="token" type="password" autocomplete="off" placeholder="sectile_…"></label><button>Connect</button></form></section>
