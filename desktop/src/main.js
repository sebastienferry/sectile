...
<section id="setup" hidden><div id="agent-offline" role="status" hidden><strong>Local agent is stopped</strong><p>Start the agent to run tasks and access your local consoles.</p></div><h1>Connect to Sectile</h1><p>Generate a pairing code in the Sectile web interface, under your profile, and paste it here. A code is single use and expires within ten minutes. A workstation you already paired can connect with its token instead.</p>
<form id="start"><label>Sectile server<input name="server" type="url" value="http://localhost:8090" required></label><label>Pairing code<input name="code" type="text" autocomplete="off" spellcheck="false" placeholder="Paste the code from the web interface"></label><label>Server token<input name="token" type="password" autocomplete="off"></label><button>Connect</button></form></section>
